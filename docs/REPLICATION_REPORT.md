...
> **⚠️ 权威版本提示(2026 更新)**:最终课程报告 [`report/report.pdf`](../report/report.pdf) 为**权威交付物**,
> 在本文基础上新增**五项扩展**(R1 FF25、R2 去 look-ahead、R3 bootstrap CI、R4 字面 η=1、EXT 广义收缩曲线),
> 并**更正**了本文若干旧表述——以 `report.pdf` 为准:① R2 表明纯 L2 的 PC 旋转 look-ahead **严格为 0**(非"<0.0005"),
> PC-sparse 全局最优 train-only Q(0.269)不低于全样本 Q(0.252);② MLP 数值经 `reproduce_all.py` 重生成为
> ridge 0.216 vs MLP 0.071(原文 0.220/0.083,结论不变:MLP 未跑赢);③ EXT 学收缩形状 η\*≈3.6 的增益 +0.04
> 落在 bootstrap 95% CI 内(含 0,不显著)。`reproduce_all.py` 现产出 fig1-8,约 2 分钟。
...
> 注:以上为 `reproduce_all.py` 重生成的当前数值(原稿曾记 0.220/0.083,结论不变:MLP 未跑赢)。
> EXT 广义收缩曲线(`gen_shrinkage.py`)从"可学收缩形状 η"角度进一步坐实这一结论——学到 η\*≈3.6 相对
> 论文固定 η=2 的增益 +0.04 落在 bootstrap 95% CI 内(含 0,不显著)。详见 `report/report.pdf` 第 6 节。
...
- **已知局限(不影响结论)**:
  - PC 旋转矩阵 Q 用**全样本** `regcov` 估计(含各 test 块),属轻微 look-ahead。**扩展 R2 已严格量化**
    (`r2_lookahead.py`,见 `report/report.pdf` 第 6.4 节):纯 L2 ridge **旋转等变**,故全样本 Q 与 train-only Q
    的 OOS 差**精确为 0**(数值 $3.3\times10^{-15}$);PC-sparse 旋转相关,但其全局最优 OOS 在 train-only Q
    下(0.269)**不低于**全样本 Q(0.252),即 look-ahead 并未抬高主线 PC 结果。(原稿"<0.0005"的粗略表述已被
    R2 的精确结论取代。)此外 EXT 的一般-η OOS 默认即用 train-only Q,本无此 look-ahead。
